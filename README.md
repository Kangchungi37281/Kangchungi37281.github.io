# Kangchungi37281.github.io

웹 사이트에 오신 것을 환영합니다.
이 사이트는 태풍,티니핑,마리오,모동숲 등을 다룰것입니다
저는 태풍에 집중하기에 태풍 관련 도구가 많습니다

이 사이트들의 게임과 위키들은 전부 ai로 제작되었습니다. 저는 html를 코딩할 방법을 모르거든요

사이트 기능 제거됨

## 트랙 메이커 링크

[윈도우 및 모바일](https://kangchungi37281.github.io/trackmakerai_v1.2.5.html)

## 주의사항

1.ai가 만들어서 오류가 있을 수 있습니다. 그렇다고 문제를 제기하지마세요. 저는 작업할 줄 모릅니다.

## 역사

주의사항:역사는 개발자만 수정 가능하며 깡충이 개발자가 입력한 역사는 제거할 수 없으며 수정만 가능합니다

2026년 5월 20일 업데이트:PC를 오늘부터 로그아웃 합니다 pc를 쓰긴 하는데 오늘부터 bing 번역 오류랑 미해결 문제들로 인해 pc만 로그아웃해야할 것 같습니다

ai가 만든 것들이다 보니까 오류가 많을 수도 있습니다. 저는 html 초보자입니다

2026년 6월 5일 업데이트:제가 드디어 크롬으로 깃허브를 로그인했습니다! 그래서 같은 pc를 다시 씁니다!

2026년 6월 30일:[바이러스 의심 사이트](https://www.natyphoon.top)에서 파일을 다운로드할겁니다. 만약 윈도우가 exe 파일 실행 후 바이러스에 걸리면 앞으로 노트북을 사용할 수 없을 수도 있습니다. 제가 한번 테스트해볼겁니다.(이 문제는 이미 해결되었습니다)
